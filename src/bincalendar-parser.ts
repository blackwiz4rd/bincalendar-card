...
export function parseReply(calendar: object, bins: object): object {
  ...
  var afterDates = Object.values(events).filter(function(d) {
      return parseDate(d.dtstart).getTime() - date.getTime() > 0;
  });

  afterDates.sort(function(a, b) {
      var distancea = Math.abs(parseDate(a.dtstart).getTime() - date.getTime());
      var distanceb = Math.abs(parseDate(b.dtstart).getTime() - date.getTime());
      return distancea - distanceb;
  });

  var bins_count = {};
  for (const i in bins) {
      var re = new RegExp(bins[i], 'g');
